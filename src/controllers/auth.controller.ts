...
const publicUserSelect = {
  id: true, name: true, email: true, username: true, phone: true, role: true,
  avatar: true, avatarPublicId: true, verified: true, createdAt: true, updatedAt: true
};
...
export const register = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const selectedRole: Role = role === "HOST" || role === "ADMIN" ? role : "GUEST";
    const existingUser = await prisma.user.findFirst({ where: { OR: [{ email }, { username }] } });
    if (existingUser) return res.status(409).json({ message: "Email or username already exists" });

    const hashedPassword = await bcrypt.hash(password, 10);
    const user = await prisma.user.create({
      data: { name, email, username, phone, password: hashedPassword, role: selectedRole },
      select: publicUserSelect
    });

    return res.status(201).json({ message: "Register successful", user, token: signToken(user.id, user.role) });
  } catch (error: any) {
...
export const login = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(200).json({ message: "Login successful", user: safeUser, token: signToken(user.id, user.role) });
  } catch (error: any) {
...
export const forgotPassword = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await prisma.user.update({
      where: { id: user.id },
      data: { resetToken: hashedToken, resetTokenExpiry: new Date(Date.now() + 1000 * 60 * 30) }
    });

    const resetUrl = `${process.env.API_URL || "http://localhost:3000"}/api/v1/auth/reset-password/${token}`;
    await sendEmail(user.email, "Password Reset", passwordResetEmail(user.name, resetUrl));
    return res.status(200).json({ message: "Password reset link generated", resetToken: token });
  } catch (error: any) {
...
export const resetPassword = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!tokenParam || !newPassword) return res.status(400).json({ message: "token and newPassword are required" });
